...
class PureTextMCQExtractor:
    ...
    def find_pure_text_multiple_choice_questions(self, text):
        """Find pure text-based MCQ questions with their A, B, C, D options"""
        questions = []
        ...
        for question_num, boundary in boundaries.items():
            ...
            if option_start != -1:
                ...
                # Only add if we have all 4 options and it's a pure text MCQ
                if (len(options) == 4 and 
                    ...
                    print(f"Found pure text MCQ {question_num}: {len(options)} options")
    ...
    def process_paper(self, qp_path, ms_path, output_file):
        """Process a complete paper to extract Q&A with pure text MCQ options"""
        ...
        # Find pure text questions with options from question paper
        questions_with_options = self.find_pure_text_multiple_choice_questions(qp_text)
        print(f"Found {len(questions_with_options)} pure text MCQs with options")
        ...
        result = {
            ...
            'summary': {
                ...
                'pure_text_mcqs_with_options': len([m for m in merged_data if m['options']]),
...
def main():
    extractor = PureTextMCQExtractor()
    ...
    winter_result = extractor.process_paper(
        ...
        'winter_pure_text_mcqs.json'
    )
    
    print("\n=== EXTRACTION COMPLETE ===")
    if summer_result:
        print(f"Summer paper: {summer_result['summary']['pure_text_mcqs_with_options']}/{summer_result['summary']['total_pairs']} pure text MCQs have options")
    if winter_result:
        print(f"Winter paper: {winter_result['summary']['pure_text_mcqs_with_options']}/{winter_result['summary']['total_pairs']} pure text MCQs have options")
