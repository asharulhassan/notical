...
class BasicPhysicsMCQExtractor:
    def __init__(self):
    ...
    def is_basic_physics_question(self, question_text):
        """Check if this is a basic physics question (allows units and measurements)"""
        ...
        # Skip if question contains complex mathematical expressions
    ...
    def is_basic_physics_options(self, options):
        """Check if options are basic physics (allows units and measurements)"""
        # Check if all options have meaningful content
        for option_text in options.values():
            if not option_text.strip() or len(option_text.strip()) < 2:
                return False
            ...
            complex_keywords = [
                'formula', 'equation', 'derive', 'calculate'
            ]
            
            if any(keyword in option_lower for keyword in complex_keywords):
                return False
            
            # Skip if options contain complex mathematical expressions
            complex_math_patterns = [
    ...
    def find_basic_physics_multiple_choice_questions(self, text):
        """Find basic physics MCQ questions with their A, B, C, D options"""
        questions = []
        ...
        for question_num, boundary in boundaries.items():
            ...
            if option_start != -1:
                ...
                # Only add if we have all 4 options and it's a basic physics MCQ
                if (len(options) == 4 and 
                    self.is_basic_physics_question(question_text) and 
                    self.is_basic_physics_options(options)):
                    ...
                    print(f"Found BASIC PHYSICS MCQ {question_num}: {len(options)} options")
    ...
    def process_paper(self, qp_path, ms_path, output_file):
        """Process a complete paper to extract Q&A with basic physics MCQ options"""
        ...
        # Find basic physics questions with options from question paper
        questions_with_options = self.find_basic_physics_multiple_choice_questions(qp_text)
        print(f"Found {len(questions_with_options)} BASIC PHYSICS MCQs with options")
        ...
        result = {
            ...
            'summary': {
                ...
                'basic_physics_mcqs_with_options': len([m for m in merged_data if m['options']]),
...
def main():
    extractor = BasicPhysicsMCQExtractor()
    ...
    summer_result = extractor.process_paper(
        ...
        'summer_basic_physics_mcqs.json'
    )
    ...
    winter_result = extractor.process_paper(
        ...
        'winter_basic_physics_mcqs.json'
    )
    
    print("\n=== EXTRACTION COMPLETE ===")
    if summer_result:
        print(f"Summer paper: {summer_result['summary']['basic_physics_mcqs_with_options']}/{summer_result['summary']['total_pairs']} BASIC PHYSICS MCQs have options")
    if winter_result:
        print(f"Winter paper: {winter_result['summary']['basic_physics_mcqs_with_options']}/{winter_result['summary']['total_pairs']} BASIC PHYSICS MCQs have options")
